...
accelerate launch --multi_gpu --num_processes $NUM_GPUS accelerate_rlaif/generate_sft_dataset.py Qwen/Qwen3-4B accelerate_rlaif/constitutions/constitution_from_doc.json 2 8 datasets/base_train.json datasets/sft_train.json
accelerate launch --multi_gpu --num_processes $NUM_GPUS accelerate_rlaif/generate_sft_dataset.py Qwen/Qwen3-4B accelerate_rlaif/constitutions/constitution_from_doc.json 2 8 datasets/base_test.json datasets/sft_test.json

echo "------------------------------------"
echo "training the SFT model on the generated dataset"
accelerate launch --multi_gpu --num_processes $NUM_GPUS accelerate_rlaif/training_sft_model.py Qwen/Qwen3-4B datasets/sft_test.json models/sft_model
...
accelerate launch --multi_gpu --num_processes $NUM_GPUS accelerate_rlaif/training_dpo_model.py datasets/dpo_train.json Qwen/Qwen3-4B logs/ models/dpo_model 8
...
accelerate launch -m lm_eval --model hf \
    --model_args pretrained=models/sft_model,tokenizer=Qwen/Qwen3-4B \
    --tasks mmlu,bbq,gsm8k \
    --batch_size 16

accelerate launch -m lm_eval --model hf \
    --model_args pretrained=models/dpo_model,tokenizer=Qwen/Qwen3-4B \
    --tasks mmlu,bbq,gsm8k \
    --batch_size 16
